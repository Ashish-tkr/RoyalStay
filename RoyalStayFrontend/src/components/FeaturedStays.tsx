...
import { Star, MapPin, ArrowRight, Wifi, Dumbbell, Utensils } from 'lucide-react';
...
function processApartment(apt: Apartment): Apartment {
  if (!apt.flats?.length) return apt;
  const cheapest = apt.flats.reduce((a, b) => b.price < a.price ? b : a);
  return {
    ...apt,
...
const AmenityIcon = ({ name }: { name: string }) => {
  ...
  if (n.includes('wifi') || n.includes('internet')) return <Wifi size={12} />;
  if (n.includes('gym') || n.includes('fitness')) return <Dumbbell size={12} />;
  if (n.includes('restaurant') || n.includes('dining')) return <Utensils size={12} />;
  return null;
};
...
const FeaturedStays = () => {
  ...
  useEffect(() => {
    const fetchApartments = async () => {
      try {
        const res = await axios.get<Apartment[]>("http://localhost:5000/api/apartments");

        const featured = res.data
          .slice(0, 3)
          .map(processApartment);
  ...
  const handleBookNow = (apt: Apartment, e: React.MouseEvent) => {
    e.preventDefault();

    openBooking({
      propertyName: apt.name,
      propertyImage: apt.coverImage,
      initialDateRange: {
        from: new Date(),
  ...
  return (
    <section className="py-28 px-8 lg:px-16" style={{ background: '#0A0805' }}>
        ...
        {!loading && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {apartments.map((apt, index) => (
              <div
                key={apt._id || index}
                onMouseEnter={() => setHovered(apt._id || null)}
                onMouseLeave={() => setHovered(null)}
                className="p-4 border rounded text-white"
              >
                <img
                  src={apt.coverImage}
                  alt={apt.name}
                ...
                <h3 className="mt-2 text-lg">
                  {apt.name.replace(/RoyalStay/gi, 'RoyalStay')}
                </h3>
                ...
                <div className="flex gap-2 mt-2">
                  {apt.flats?.map(flat => (
                    <span key={flat.type} className="text-xs border px-2">
                      {flat.type}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-4 flex justify-between items-center">
                  <span>₹{(apt.minPrice || 0).toLocaleString()}</span>

                  <button
                    onClick={(e) => handleBookNow(apt, e)}
